...
// Value lives in `splashGradients.ts`, also used by the dormant
// `AuthLoadingScreen`'s own static backdrop (App.tsx) — re-exported under
// this file's own name since every OTHER constant here is scoped to "this
// specific FAB," not the splash-gradient concept generally.
 ...
 *
 * Review fix: this button's own first-load splash (a giant circle
 * collapsing onto it on first mount each session, the same mechanic
 * `OnboardingScreen`/`ReturningUserLoadingScreen` use for their own
 * wordmark) is REMOVED per explicit request — the "+" now just renders
 * at its normal resting state immediately, every mount, with no special
 * first-load treatment. `firstLoadSplash.ts`'s one-shot flag is
 * untouched — `ReturningUserLoadingScreen` is still its sole consumer,
 * for its own, separate "play once per session" need.
 */
